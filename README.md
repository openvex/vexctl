...
VEX documents are often edited by hand, and `vexctl` ignores data it does not
recognize: a misspelled field name usually turns into a statement that silently
says less than its author meant. The `vexctl validate` subcommand reads a
document and reports what is wrong with it, instead of stopping at the first
problem:
