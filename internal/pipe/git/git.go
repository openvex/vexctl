// TODO: Strip this down to what we need
package git
...
import (
	...
	// NOTE: We probably want to write our own context
	"github.com/goreleaser/goreleaser/pkg/context"
	"github.com/openvex/vexctl/internal/git"
	"github.com/openvex/vexctl/internal/pipe"
)
...
func getGitInfo(ctx *context.Context) (context.GitInfo, error) {
	...
	tag, err := getTag(ctx, ctx.Config.Git.IgnoreTags)
	if err != nil {
	...
	previous, err := getPreviousTag(ctx, tag, ctx.Config.Git.IgnoreTags)
	if err != nil {
...
func getPreviousTag(ctx *context.Context, current string, excluding []string) (string, error) {
	for _, fn := range []func() ([]string, error){
		getFromEnv("VEXCTL_PREVIOUS_TAG"),
		func() ([]string, error) {
