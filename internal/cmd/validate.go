...
The validate subcommand reads one or more OpenVEX documents and reports what
is wrong with them. It is meant for documents written or edited by hand, where
a misspelled field or a malformed package URL is easy to introduce and hard to
notice: %s ignores data it does not recognize, so a typo usually turns into
a statement that silently says less than its author meant.
