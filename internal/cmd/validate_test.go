...
import (
	...
	"github.com/openvex/vexctl/pkg/validate"
)
...
// runValidate runs the validate subcommand and returns what it printed. The
// flags of the shared root command keep their values between runs, so every
// run states the ones it cares about.
...
// runTextValidate runs the validate subcommand with the default report and
// warnings tolerated, which is what most cases want.
func runTextValidate(t *testing.T, args ...string) (string, error) {
	t.Helper()
	return runValidate(t, append([]string{"--format=text", "--strict=false"}, args...)...)
}
...
func TestValidateCommand(t *testing.T) {
	...
	t.Run("warnings alone do not fail", func(t *testing.T) {
		out, err := runTextValidate(t, warns)
		require.NoError(t, err)
